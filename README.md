# note

個人用の学習ノート置き場

## Contents

- [programming/](./programming/) - プログラミング言語・フレームワーク
  - [CSharp.md](./programming/CSharp.md) - C# の型・クラス・LINQ・非同期など
  - [TypeScript.md](./programming/TypeScript.md) - TypeScript の基本、型、ビルド、周辺ツールなど
  - [React.md](./programming/React.md) - React / TSX / Hooks などのメモ
  - [Next.js.md](./programming/Next.js.md) - Next.js (App Router) / ルーティング / Server Components など
  - [Go.md](./programming/Go.md) - Go の基本構文・型・並行処理・標準ライブラリなど
  - [Python.md](./programming/Python.md) - Python の基本構文・データ構造・クラス・標準ライブラリなど
  - [Vite.md](./programming/Vite.md) - Vite のセットアップ・設定・環境変数・プラグインなど
  - [Tailwind.md](./programming/Tailwind.md) - Tailwind CSS のユーティリティクラス・レスポンシブ・カスタマイズなど
- [architecture/](./architecture/) - 設計・アーキテクチャ
  - [REST_API_Design.md](./architecture/REST_API_Design.md) - REST API 設計・OpenAPI・CRUD・ステータスコードなど
  - [AWS_CDK.md](./architecture/AWS_CDK.md) - AWS CDK v2 / App・Stack・Construct・よく使うリソースなど
  - [AWS_CloudFormation.md](./architecture/AWS_CloudFormation.md) - CloudFormation / テンプレート構造・組み込み関数・スタック操作など
- [godot/](./godot/) - Godot ゲームエンジン
  - [Godot.md](./godot/Godot.md) - Godot 4.x / ノード・シーン・GDScript・2D ゲーム基礎など
  - [Godot_CSharp.md](./godot/Godot_CSharp.md) - Godot × C# / GDScript との比較・セットアップ
- [unity/](./unity/) - Unity ゲームエンジン
  - [Unity2D.md](./unity/Unity2D.md) - Unity 6 / 2Dゲーム制作・Sprite・Tilemap・2D物理・UIなど
- [tools/](./tools/) - ツール・環境
  - [ClaudeCode.md](./tools/ClaudeCode.md) - Claude Code の使い方・設定・Tips
  - [Codex.md](./tools/Codex.md) - OpenAI Codex CLI の使い方・承認モード・設定など
  - [CodingAgentExtensions.md](./tools/CodingAgentExtensions.md) - コーディングエージェント拡張機能のメモ
  - [Markdown.md](./tools/Markdown.md) - Markdown 記法・GFM 拡張のリファレンス
  - [Mermaid.md](./tools/Mermaid.md) - Mermaid 図表記法のリファレンス（フローチャート・シーケンス図など）

## 方針

- 内容が古くなったら適宜更新する

## メモ

Markdown ファイルはそのままエディタやプレビューで読む想定<br>
必要に応じてタグや作成日、更新日を front matter に記載
