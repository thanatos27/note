...
Next.jsは、ReactをベースにしたWebアプリケーションフレームワーク。Reactだけでは自分で組み合わせる必要があるルーティング、サーバーサイドレンダリング、静的サイト生成、画像最適化、APIエンドポイントなどを、あらかじめ使いやすい形で提供してくれる。

特にApp Routerでは、ファイルとフォルダの構成からページやレイアウトを定義でき、Server ComponentsやServer Actionsを使って、画面表示とサーバー側の処理を近い場所にまとめられる。ブログ、ダッシュボード、ECサイト、管理画面など、SEOやパフォーマンスも意識したReactアプリを作りたいときに向いている。
